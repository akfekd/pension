...
public class SpotDAOImpl implements SpotDAO {
	...
	public int insertSpot(SpotDTO dto) throws SQLException {
		...
		try {
			sql="INSERT INTO spot (num, userId, content,subject, created, hitCount,imageFilename) "
	...
	public int updateSpot(SpotDTO dto) throws SQLException {
		 int result=0;
	      ...
	      try {
	         sql="UPDATE spot SET subject=?, content=?, imageFilename=? WHERE num=?";
	...
	public int deleteSpot(int num, String userId) throws SQLException {
	      ...
	      try {
	         sql="DELETE FROM spot WHERE num=?";
	...
	public int updateHitcount(int num) {
		...
		try {
			sql = "UPDATE spot SET hitCount = hitCount+1 WHERE num = ? ";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, num);
			result = pstmt.executeUpdate();
	...
	public SpotDTO readSpot(int num) {
	      ...
	      try {
	         sql = "SELECT num, m.userId, userName, subject, content, hitCount, r.created, imageFilename FROM spot r JOIN member1 m "
	...
	public SpotDTO preSpot(int num, String condition, String keyword) {
		SpotDTO dto=null;
	        ...
	        try {
	        	if(keyword.length() != 0) {
	                sb.append("SELECT num, subject, content FROM spot r  ");
	                sb.append("  JOIN member1 m ");
	    			sb.append(" ON r.userId = m.userId ");
	    			if(condition.equals("all")) {
	    				sb.append(" WHERE ( INSTR(subject, ?) >=1 OR INSTR(content,?) >= 1) ");
	                ...
	                sb.append(" SELECT num, subject FROM spot ");
	                sb.append(" WHERE num > ? ");
	                sb.append(" ORDER BY num ASC ");
	                sb.append(" FETCH  FIRST  1  ROWS  ONLY ");
