...
public class BoardDAOImpl implements BoardDAO {
	...
	public int insertBoard(BoardDTO dto) throws SQLException {
		...
		try {
			sql="INSERT INTO bbs (userId, subject, content, hitCount, created)"
				+"VALUES (?, ?, ?, 0, SYSDATE)";
	...
	public int dataCount() {
		...
		try {
			sql=" SELECT COUNT(*) cnt FROM bbs b "
					+ " JOIN member1 m ON b.userId = m.userId "; // 이 줄은 없어도 문제가 생기지는 않음
			pstmt=conn.prepareStatement(sql);
	...
	public int dataCount(String condition, String keyword) {
		...
		try {
			...
			if(condition.contentEquals("created")) {
	...
	public List<BoardDTO> listBoard(int offset, int rows) {
		...
		try {
			...
			sb.append("		  m1.userName, b.created, ");
			sb.append("		  b.hitCount ");
			sb.append(" FROM member1 m1, bbs b ");
			sb.append(" WHERE m1.userId=b.userId ");
	...
	public List<BoardDTO> listBoard(int offset, int rows, String condition, String keyword) {
		...
		try {
			...
			while(rs.next()) {
				BoardDTO dto=new BoardDTO();
				dto.setNum(rs.getInt("num"));  // dto.setNum(rs.getInt(1));
	...
	public BoardDTO readBoard(int num) {
		...
		try {
			sql="SELECT num, b.userId, userName, subject, content, hitCount, created "
					+"  FROM bbs b JOIN member1 m ON b.userId = m.userId WHERE num=?";
			pstmt=conn.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs=pstmt.executeQuery();
			if(rs.next()) {
